...
class Short extends App{	
    ...
    // If require registration
		...
		if(isset($this->user) && $this->user->id && (!isset($array["password"]) || empty($array["password"])) &&  (!isset($array["custom"]) || empty($array["custom"])) && empty($array["type"])){
			if($data = $this->db->get("url",array("url"=>"?","userid"=>"?","location"=>"?","pass"=>"?", "devices" => "?", "pixels" => "?"), array("limit"=>"1"), array($url, $this->user->id, $countries, "", $devices, ""))){
