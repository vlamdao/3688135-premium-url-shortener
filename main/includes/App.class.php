...
class App{
	...
	protected function user(){
		...
		if($this->logged()){
			...
			$action = array("edit","delete","archive","expired","bundles","splash","settings","logout","verify","search","server","export","overlay","pixels", "tools", "cancel","terminate","membership","domain","teams");
	...
	protected function webhook(){
		...
		$fn = "webhook_{$this->do}";
		if(in_array($this->do, ["slack"]) && method_exists(__CLASS__, $fn)){
			return $this->$fn();
		}				
	...
	protected function menu($option=array()){
		$menu='<div class="navbar-collapse collapse">';
          ...
          $menu.="<li class='dropdown'>
					          <a href='".Main::href('user')."'><img src='{$this->avatar($this->user)}' alt=''></a>
					          <ul>
      ...
      if(!empty($option) && is_array($option)){
		...
		$menu.='</ul>';
			$menu.='<h3 class="hidden-xs">'.e("Membership");
              	...
              	$menu.='<span class="label label-primary pull-right">'.e($this->user->plan->name).' '.($this->user->trial ? e("Free Trial") : '').'</span>';
              }else{
              	$menu.='<span class="label label-primary pull-right">'.e($this->user->plan->name).' '.($this->user->trial ? e("Free Trial") : '').'</span>';
              }	              	
	  	$menu.='</h3>';
